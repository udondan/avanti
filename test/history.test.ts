...
describe('HistoryManager.updateFileSudo', () => {
  it('sets sudo on a tracked file and getFileMeta reflects the change', () => {
    const f = path.join(tmpDir, 'tracked.txt');
    fs.writeFileSync(f, 'content', 'utf8');
    const h = makeManager();
    h.ensureStorageDir();
    const p = h.openPullSession();
    const { fileRef } = h.stageFileVersion(p, f, buf('content'), false);
    h.closePullSession(p, '/project/.avanti.yml', [fileRef]);

    h.updateFileSudo(f, true);
    expect(h.getFileMeta(f)?.sudo).toBe(true);
  });

  it('updates sudo from one user to another', () => {
    const f = path.join(tmpDir, 'tracked2.txt');
    fs.writeFileSync(f, 'content', 'utf8');
    const h = makeManager();
    h.ensureStorageDir();
    const p = h.openPullSession();
    ...
    h.updateFileSudo(f, 'bob');
    expect(h.getFileMeta(f)?.sudo).toBe('bob');
  });

  it('clears sudo when called with undefined', () => {
    const f = path.join(tmpDir, 'tracked3.txt');
    fs.writeFileSync(f, 'content', 'utf8');
    const h = makeManager();
    h.ensureStorageDir();
    const p = h.openPullSession();
    ...
    h.updateFileSudo(f, undefined);
    expect(h.getFileMeta(f)?.sudo).toBeUndefined();
  });
  ...
  it('is a no-op for an untracked file', () => {
    ...
    h.ensureStorageDir();
    // Should not throw
    expect(() => h.updateFileSudo('/never/tracked.txt', true)).not.toThrow();
  });
});
